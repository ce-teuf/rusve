...
pub fn init_envs() -> Result<Env> {
    Ok(Env {
        ...
        data_database_url: std::env::var("DATA_DATABASE_URL").context("DATA_DATABASE_URL is not set")?,
...
pub fn connect_to_db(url: &str) -> Result<Pool> {
    let tokio_config = tokio_postgres::Config::from_str(url)?;
...
pub fn auth(
    ...
    let token = match metadata.get("x-authorization") {
        ...
        None => {
            ...
            return Err(tonic::Status::unauthenticated("Missing authorization token"));
        }
    };
    let token = token
        .to_str()
